...
def g():
    ...
    y_pred_titanic = model.predict(X_test)

    # Compare predictions (y_pred) with the labels in the test set (y_test)
    metrics_titanic = classification_report(y_test, y_pred_titanic, output_dict=True)
    results_titanic = confusion_matrix(y_test, y_pred_titanic)

    # Create the confusion matrix as a figure, we will later store it as a PNG image file
    df_cm_titanic = pd.DataFrame(results_titanic, ['Survived', 'Died'],
                         ['Pred Survived', 'Pred Died'])
    cm_titanic = sns.heatmap(df_cm_titanic, annot=True)
    fig = cm_titanic.get_figure()
    ...
    titanic_model = mr.python.create_model(
        name="titanic_modal", 
        metrics={"accuracy" : metrics_titanic['accuracy']},
